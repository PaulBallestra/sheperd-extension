...
class SheperdPopupApp {
  ...
  bindEvents() {
    ...
    document.addEventListener(SHEPERD_EVENTS.TABS_UPDATED, async (event) => {
      ...
      // Don't refresh for optimistic single tab updates - they handle their own UI
      if (
        action !== "refresh" &&
        action !== "tab_closed" &&
  ...
  async handleRealTimeTabChange(eventType, data) {
    ...
    try {
      ...
      // Skip refresh for tab removal events - they're handled optimistically by categories component
      if (eventType === "tab_removed") {
        console.log(
          "⚡ Skipping real-time refresh for tab removal (handled optimistically)"
        );
        return;
      }
      ...
      const expandedCategories =
        this.components.categories.getExpandedCategories();
